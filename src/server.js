...
function createApp({ store, realtime }) {
  ...
  app.post('/api/messages', authMiddleware, (req, res) => {
    try {
      ...
      realtime.broadcastToServer(message.serverId, { event: 'message.created', data: message });
  ...
  app.post('/api/presence/heartbeat', authMiddleware, (req, res) => {
    try {
      ...
      const heartbeat = store.heartbeat({ serverId, userId: req.userId });
      realtime.broadcastToServer(serverId, { event: 'presence.updated', data: heartbeat });
  ...
  app.get('/api/presence', authMiddleware, (req, res) => {
    try {
      ...
      const members = store.getPresence({ serverId, userId: req.userId });
