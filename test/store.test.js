...
const { InMemoryStore, THIRTY_DAYS_MS, PRESENCE_STALE_MS } = require('../src/store');
...
test('presence heartbeat reports online then offline when stale', () => {
  const { store, setNow } = makeStore();
  const base = Date.UTC(2026, 0, 31);

  setNow(base);
  const heartbeat = store.heartbeat({ serverId: 's1', userId: 'u2' });
  assert.equal(heartbeat.userId, 'u2');

  let presence = store.getPresence({ serverId: 's1', userId: 'u1' });
  assert.equal(presence[0].status, 'online');

  setNow(base + PRESENCE_STALE_MS + 1);
  presence = store.getPresence({ serverId: 's1', userId: 'u1' });
  assert.equal(presence[0].status, 'offline');
});
