...
- [x] ~~BUG: Furniture on void cells breaks avatar movement~~ — fixed (filter sortedFurniture by isInBounds + isInMask)
- [x] ~~BUG: Electron blank screen in production~~ — fixed (Vite base was '/' absolute, changed to './' relative for file:// compat)
- [x] ~~Boosted themed backgrounds~~ — sci-fi starfield bigger/brighter stars, retro-arcade neon grid more visible
- [x] ~~GitHub Release v0.2.0~~ — Windows installer published at github.com/Calum136/AudioChat/releases/tag/v0.2.0
- [ ] BUG: Furniture cannot be added from palette (existing bug, not yet fixed)
- [ ] Distribution: GitHub release is private — need public download link (Netlify static asset, Google Drive, or make repo public)
- [x] ~~Electron auto-update~~ — electron-updater + GitHub Releases publish config, UpdateNotification toast
- [x] ~~Window dragging broken~~ — fixed (added ::before drag bar on landing, backgroundThrottling:false)
- [x] ~~App unfocus breaks room joining~~ — fixed (backgroundThrottling:false + presence refresh on focus)
- [x] ~~Friends panel redesign~~ — replaced tabs with "..." dropdown (Requests/Blocked/Add Friend sub-views)
- [x] ~~Presence bug (friend invites not working)~~ — fixed (updatePresenceRoom used wrong userId from presence state)
- [x] ~~Joined rooms not saved~~ — room_members table, getUserRooms fetches owned + joined, Leave button for non-owned
- [x] ~~Edit mode permissions~~ — owner + admin can edit (room_members.role column, promote/demote in participant panel)
- [x] ~~Fantasy tavern layout broken~~ — removed L-shape, full 10x8 rectangle, new barcounter furniture (3x1, 3 seats)
- [x] ~~Multiple app icons on taskbar~~ — proper .ico, consistent icon config in package.json + main.cjs + NSIS
...
- [ ] PENDING: Apply migration 006_room_members.sql to Supabase (run SQL in dashboard)
...
- **Apr 8, 2026:** Bug fixes + Electron release. (1) Fixed furniture on void cells — sortedFurniture now filters by isInBounds + isInMask, so furniture stuck on masked-out cells after theme change no longer renders or breaks avatar movement. (2) Boosted themed backgrounds — sci-fi starfield stars enlarged (2-3px), brighter opacity, 25 star dots (up from 15); retro-arcade neon grid opacity bumped 0.03→0.08, grid size 20→24px, pulse range widened. (3) Fixed Electron blank screen — Vite base was conditionally `'./'` only when `ELECTRON=true` env var set, but electron:build script never set it. Changed to unconditional `base: './'` which works for both Netlify and Electron file:// protocol. (4) Built and published GitHub Release v0.2.0 with Windows NSIS installer (~90MB). Note: repo is private so release link not publicly accessible — need alternative distribution (Netlify static asset, cloud storage, or public repo). electron-builder had persistent file locking issues on win-unpacked/app.asar — workaround was building to fresh output directories.
- **Apr 8, 2026:** 9-issue batch fix from friend testing feedback. (1) Auto-update: installed electron-updater, added autoUpdater to main.cjs with IPC events, UpdateNotification.jsx toast component, GitHub Releases publish config in package.json. (2) Window dragging: added fixed ::before drag bar on landing page, ensured all interactive elements have no-drag. (3) Background focus: added backgroundThrottling:false to webPreferences, forward focus/blur events via IPC, re-track presence on focus. (4) Friends panel redesign: replaced tabs with "..." dropdown menu showing Requests/Blocked/Add Friend sub-views, added Invite button for online friends. (5) Presence bug fix: updatePresenceRoom was reading userId from first key in presence state (wrong user) — now stores _presenceUserId on connect. (6) Room membership persistence: new room_members table + migration 006, getUserRooms fetches both owned + joined rooms, rooms saved on join, "Leave" button for non-owned rooms. (7) Admin permissions: room_members.role column (member/admin), isAdmin state in roomStore, canEdit = isOwner || isAdmin for edit/theme/palette, promote/demote in participant panel. (8) Fantasy tavern: removed L-shape mask (now full 10x8 rectangle), added barcounter furniture (3x1 tile, 3 seats, warm wood sprite). (9) App icons: generated proper .ico from PNG, updated package.json + main.cjs to use .ico on Windows, added installerIcon/uninstallerIcon to NSIS config. PENDING: Apply migration 006_room_members.sql to Supabase.
- **Apr 8, 2026:** Installed and configured Stitch MCP server (Google's UI generation tool) via `claude mcp add stitch --transport http https://stitch.googleapis.com/mcp --header "X-Goog-Api-Key: ..."` at user scope. Verified connection — found existing Stitch project "Virtual Hangout Space" (project ID: 3957935708045886813) with 8 screens using "The Neon Nocturne" dark design system. Attempted to generate 5 new Sidequest-specific screens (Login, Home, Lobby, Room, Settings) but Stitch generation consistently timed out through MCP HTTP transport — check stitch.withgoogle.com directly as screens may have been created in background. Gold (#D4AF37) confirmed as primary accent color going forward, replacing current purple/teal. Drafted full UI alignment plan covering all 5 screens — includes new Lobby.jsx component, global CSS token overhaul, Space Grotesk + Manrope font swap, and per-component changes. Plan approved in principle but implementation deferred — ready for handoff to Opus coding session when resumed.
- **Apr 20, 2026:** Lobby redesign + bug fix batch. (1) Home screen redesigned as game lobby hub — replaced dash-header+quick-bar toggle pattern with a 2-row glass panel header (brand/user top row, always-visible create/join inputs bottom row). New LobbyRoomCard component: wider cards at minmax(220px, 1fr), 80px preview image, themed label + active count badge, bold room name, join code (click-to-copy) + gold "Enter →" CTA, × dismiss on hover. Empty state has two CTA buttons that focus the corresponding input. #D4AF37 gold as primary accent. (2) Critical channel naming bug fixed — Landing was opening presence channels named `room:${id}` which collided with the main room channel from roomChannel.js. Supabase reused the channel so the SUBSCRIBED callback never fired → infinite loading spinner, no character appearing in room, couldn't sit. Fixed by renaming Landing presence channels to `lobby:${id}`. (3) Settings panel given fixed height (min(600px, 85vh)) so nav sidebar no longer shifts when switching between tabs of different content height. (4) Removed opacity 0.85→1 hover animation on room card preview images (looked like image was changing/janky). (5) Added glass panel backgrounds + borders to lobby-header and lobby-rooms-area for clear visual segmentation between header, rooms, and friends panel. Commits: 80e2a4b (lobby redesign), fbc13c6 (bug fixes).
